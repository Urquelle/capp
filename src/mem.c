...
#else
    result->mem = VirtualAlloc(NULL, size, MEM_RESERVE|MEM_COMMIT, PAGE_READWRITE);
#endif
...
void
mem_arena_init(Mem_Arena *arena) {
    arena->num_pages = 1;
    arena->curr_page = mem_page(MIN_PAGE_SIZE);
    arena->pages     = arena->curr_page;
}
